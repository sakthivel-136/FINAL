...
with st.expander("📄 Export Chat and Email"):
    ...
    if st.button("Send Email"):
        if recipients and export_type:
            if export_type == "PDF Bilingual":
                file_path = export_chat_to_bilingual_pdf()
            else:
                file_path = export_chat_to_text_file(export_type)
            result = send_email(recipients, f"KCET Chat Export ({export_type})", f"Attached is your KCET chat export in {export_type} format.", file_path)
            if result is True:
