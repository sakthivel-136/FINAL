...
def run_chatbot():
    with st.spinner("🔄 Loading KCET Assistant..."):
        time.sleep(1.5)  # entrance animation delay

    # Background music autoplay (optional for users)
    try:
    ...
    for speaker, msg, role in st.session_state.original_log:
        ...
        st.markdown(f"""
            <div style='background-color:{color}; padding:10px; margin:10px; border-radius:10px; text-align:{align}; transition: all 0.4s ease;'>
                <b>{speaker}</b>: {msg}
            </div>
        """, unsafe_allow_html=True)
